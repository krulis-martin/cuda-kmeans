...
KMCUDAResult kmeans_cuda_lloyd(
    ...
    udevptrs<uint32_t> *assignments, int *iterations = nullptr, int maxIterations = 0) {
  std::vector<uint32_t> shmem_sizes;
...
KMCUDAResult kmeans_cuda_yy(
    ...
    udevptrs<float> *drifts_yy, udevptrs<uint32_t> *passed_yy, int &iter, int maxIters) {
  if (h_yy_groups_size == 0 || YINYANG_DRAFT_REASSIGNMENTS <= tolerance) {
    ...
    return kmeans_cuda_lloyd(
        ...
        assignments_prev, assignments, &iter, maxIters);
  }
  ...
  RETERR(kmeans_cuda_lloyd(
      ...
      centroids, ccounts, assignments_prev, assignments, &iter, maxIters));
  ...
  );
  {
    ...
    RETERR(kmeans_init_centroids(
		kmcudaInitMethodRandom, nullptr, h_clusters_size, h_features_size,
