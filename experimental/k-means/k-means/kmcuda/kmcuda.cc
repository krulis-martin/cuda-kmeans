...
KMCUDAResult kmeans_init_centroids(
  ...
  switch (method) {
    ...
    case kmcudaInitMethodRandom: {
      ...
      std::shuffle(chosen.begin(), chosen.end(), generator);
...
KMCUDAResult kmeans_cuda(
    ...
    uint32_t *assignments, float *average_distance, int& iterations) {
  ...
  RETERR(kmeans_cuda_yy(
      ...
      &device_centroids_yy, &device_bounds_yy, &device_drifts_yy, &device_passed_yy, iterations, iterations),
